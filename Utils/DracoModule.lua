...
local Owner = "VuzChanz"
local Repository = "ChizChanz"
...
local function VuzChanzAdd(Name, Module)
    do VuzChanzUtils[Name] = Module()
        return VuzChanzUtils[Name]
    end
end

local VuzChanzUI = fetch("Utils/VuzChanzUi.lua")
...
VuzChanzUtils.VuzChanzUI = VuzChanzUI

VuzChanzAdd("Connections", function()
    local Connections = {}
...
VuzChanzAdd("Configurations", function()
    local Configurations = {}
    local Files = "ChizChanz"
    ...
    Configurations.Files = Files or "ChizChanz"
...
VuzChanzAdd("Parallels", function()
    local Parallels = {}
        ...
        local function RunQueue(Options)
            ...
            if not Success then
                ...
                task.delay(3, function()
                    if _ENV.error_message then
                        _ENV.error_message.Text = "- ChizChanz Model -\nStart Refresh Options ..."
...
VuzChanzAdd("Plugins", function()
    local Plugins = {
        NewTabs = true
    }

    local Configurations = VuzChanzUtils.Configurations
    local Parallels = VuzChanzUtils.Parallels
    local Others = VuzChanzUtils.Others

    local Enabled, Options = Parallels.Options()

    function Plugins:Window(Info)
        VuzChanzUI['Base'] = VuzChanzUI:Window(Info or {})
        return VuzChanzUI['Base']
    end

    function Plugins:NewPage(Info)
        return VuzChanzUI['Base']:NewPage({
            Title = Info[1],
            Desc = Info[2],
            Icon = Info[3]
        })
    ...
    function Plugins:Community()
        local Community = Plugins:NewPage({ "Community", "Join our community", 115960025411300 }) do
            Community:Section("Community") do
                ...
                local Banner = VuzChanzUtils.Asset:Get("DRACOCHANZ.png") do
    ...
    function Plugins:Managers()
        local Managers = Plugins:NewPage({ "Managers", "Managers Options", 134261589888025 }) do
            ...
            Managers:Section("Configurations") do
                VuzChanzUI:SizeSlider(Managers, Plugins)
...
do
    Settings = VuzChanzUtils.Configurations:Load()
    VuzChanzUtils.Settings = Settings
end

return VuzChanzUtils
